...
В методе-обработчике в общем-то тоже ничего необычного. Все то, что мы рассмотрели выше в [параграфе про read_entity](#iwbepif_v4_dp_basicread_entity), но только наоборот:
```abap
...
## Метаданные
Для SAP Fiori иногда нужно передавать кастомные метаданные для модели, например чтобы скрыть поле в SAPUI или как-то изменить его отображение. Для этого можно использовать следующий код:
```abap
DATA(lo_anno_m) = io_model->get_annotation_model( ).
DATA(lo_vocab) = lo_anno_m->create_vocabulary_reference( iv_vocab_id      = '/IWBEP/VOC_XJG'
                                                            iv_vocab_version = '0001' ).
lo_vocab->create_include( iv_namespace = 'com.sap.vocabularies.UI.v1'
                            iv_alias     = 'Z__UI' ).
DATA(lv_target) = |{ zif_hcm_cs_sessions~mc_entity_types-participants-ext }| &&
    |/{ zif_hcm_cs_sessions~mc_entity_types-participants-props-objid-ext }|.
DATA(lo_anno_t) = lo_anno_m->create_annotations_target( iv_target = lv_target ).
DATA(lo_anno) = lo_anno_t->create_annotation( iv_term_namespace = 'com.sap.vocabularies.UI.v1'
                                                iv_term_name      = 'Hidden' ).
DATA(lo_anno_val) = lo_anno->create_simple_value( ).
lo_anno_val->set_boolean( iv_value = abap_true ).
```
Здесь мы определили аннотацию UI.Hidden: true для элемента Objid типа сущности Participant. Следует обратить внимание, что для аннотации указываются "внешние" EDM имена, а не внутренние, которые указываются при создании типа сущности. В *lv_target* указывается полный внешний путь до элемента аннотации разделяемый через '/'. Более подробно можно посмотреть в классе */iwbep/cl_v4_sample_bas_model* например в методе *create_ui_anno_product*.
## Нюансы
Несмотря на удобство фреймворка, конкретно в нашей системе (750) у него есть недоработки. Главная о которой стоит упомянуть, так как с ней столкнутся все, кто будут пытаться перенести запрос с группой сервисов в другую систему, заключается в неправильном создании ключей для переноса настроечных таблиц, содержащих записи о созданной группе сервисов. Во-первых, для записи в настроечный запрос нужно быть владельцем запроса (не задачи) - особенность сервисных групп. Во-вторых, при создании/регистрации группы сервисов в запись о переносе добавляется 3 объекта:
...
В таблицу */IWFND/C_V4_MSGT* не записывается **sprsl** для русского языка, и нужно добавлять ключ вручную. На скриншоте ниже я уже добавил русский язык:
![msgt_key.png](https://github.com/Via04/odatav4-guide/blob/master/static/msgt_key.png)
А в таблицу /IWFND/C_V4_RSAG ключ пишется не полностью, нужно добавить "\*" в поле **assignment_id** ключа переноса (если родовой ключ assignment_id не заполняется, то можно удалить добавленный автоматически ключ и добавить вручную, тогда родовой ключ автоматом будет заполнен):
![rsag_key.png](https://github.com/Via04/odatav4-guide/blob/master/static/rsag_key.png)
## Очистка кэша сервиса.
Для очистки кэша достаточно вызвать запрос $metadata для сервиса и он автоматически перед выполнением очистит кэш в системе, с которой запрос выполнялся. То есть, для того, чтобы очистить кэш на фронте например надо выполнить запрос с $metadata на фронтовой системе.